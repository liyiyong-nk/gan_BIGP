...
class PairedImageDataset(Dataset):
    def __init__(self, dataset_dir, soft_data_dir, mode='A2B', transforms_=None):
        ...
        if mode in ['A2B']:
            ...
            # img_files_B =  os.listdir(path_B)
            # img_files_A = []
            # import re
            # for i in range(len(img_files_B)):
            #     img_files_A.append(path_A + '/' + re.split(r'_fake', img_files_B[i])[0] + '.jpg')
            #     img_files_B[i] = path_B + '/' + img_files_B[i]
            # self.files_A = img_files_A
            # self.files_B = img_files_B
        else:
